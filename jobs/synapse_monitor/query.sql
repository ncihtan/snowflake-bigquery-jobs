...
SELECT
  ...
  n.NAME AS FILE_NAME,
  n.CHANGE_TYPE,
  n.MODIFIED_BY AS MODIFIED_BY_ID,
  ...
  c.NAME AS GPARENT_NAME,
  c.NODE_TYPE AS GPARENT_TYPE,
  n.NODE_TYPE,
  n.CREATED_ON
...
LEFT JOIN SYNAPSE_DATA_WAREHOUSE.SYNAPSE.NODE_LATEST b
  ON n.PARENT_ID = b.ID
LEFT JOIN SYNAPSE_DATA_WAREHOUSE.SYNAPSE.NODE_LATEST c
  ON b.PARENT_ID = c.ID
LEFT JOIN SYNAPSE_DATA_WAREHOUSE.SYNAPSE.USERPROFILE_LATEST u
  ON n.MODIFIED_BY = u.ID
WHERE n.MODIFIED_ON >= DATEADD(day, -{DAYS_BACK}, CURRENT_TIMESTAMP())
  AND n.NODE_TYPE IN ('file', 'recordset')
  AND n.MODIFIED_BY NOT IN (
    SELECT MEMBER_ID FROM SYNAPSE_DATA_WAREHOUSE.SYNAPSE.TEAMMEMBER_LATEST
    WHERE TEAM_ID = 3391844  -- HTAN_DCC
  )
ORDER BY n.MODIFIED_ON DESC;