...
def format_simple_slack_message(results):
    """
    Create a simpler text-based Slack message with Synapse links
    Each user-project-folder combination gets its own line
    """
    if not results:
        return {
    ...
    # Build message
    lines = ["📊 *Your daily HTAN Synapse activity update:*\nTESTING ONLY\n"]

    # Sort by username first, then by project, then by folder
    sorted_activities = sorted(
        user_project_folder_activity.items(), key=lambda x: (x[0][0], x[0][1], x[0][2])
    )

    for (username, project_name, folder), count in sorted_activities:
        # Create user and project links
        ...
        # Format the line based on folder location
        if folder == "root":
            location = f"the {project_link} project"
        else:
            location = f"_{folder}_ of the {project_link} project"

        line = f"• {user_link} created {count} item{'s' if count != 1 else ''} in {location}"
        lines.append(line)

    lines.append(
        f"\n🔗 <https://www.synapse.org/#!Profile:dashboard|View full activity on Synapse>"
    )
